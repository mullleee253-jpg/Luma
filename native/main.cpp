...
bool pack(const std::string& source, const std::string& destination) {
  ...
  std::array<uint64_t, 256> frequencies{}; for (uint8_t byte : input) ++frequencies[byte]; std::array<uint8_t, 256> lengths{}; makeLengths(frequencies, lengths); const Bytes compressed = encode(input, canonicalCodes(lengths)); const bool raw = compressed.size() >= input.size();
  std::ofstream output(destination, std::ios::binary); if (!output) { std::cerr << "Cannot create output file\n"; return false; }
  output.write(magic.data(), 8); output.put(static_cast<char>(raw)); output.put(0); writeLittle<uint64_t>(output, input.size()); writeLittle<uint32_t>(output, crc32(input)); for (uint8_t length : lengths) output.put(static_cast<char>(length));
  if (raw) output.write(reinterpret_cast<const char*>(input.data()), static_cast<std::streamsize>(input.size())); else output.write(reinterpret_cast<const char*>(compressed.data()), static_cast<std::streamsize>(compressed.size()));
  if (!output) return false; const double ratio = input.empty() ? 1.0 : static_cast<double>(output.tellp()) / input.size(); std::cout << "Packed " << input.size() << " -> " << output.tellp() << " bytes (" << std::fixed << std::setprecision(2) << ratio * 100.0 << "% of original)" << (raw ? "; stored raw" : "") << "\n"; return true;
}
