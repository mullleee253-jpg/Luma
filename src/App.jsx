  ...
  const compress = async (file) => {
    ...
    try {
      if (file.type.startsWith('image/')) {
        ...
        const { blob, info } = await buildSmartImageBlob(file, compact, enhance)
      ...
      setFiles((current) => current.map((item) => item.id === file.id ? { ...item, status: 'done', outputUrl: url, outputName: `luma-${file.name.replace(/\.[^.]+$/, '')}.mp4`, outputSize: blob.size } : item))
      setRunning(null)
    ...
    <main className="workbench">
      ...
      <section className="drop-panel">
        <div className="drop-copy">
          ...
          <p>{mode === 'compact' ? 'Сильное сжатие с умной обработкой деталей: уменьшение размера без заметного падения качества.' : 'Бережный режим сохраняет исходные пиксели и оптимизирует без агрессивного ухудшения.'}</p>
      ...
      <section className="files-section">
        ...
        {files.length ? <div className="file-table">
          ...
          {files.map((file) => <div className="file-line" key={file.id}><span className="file-name"><i className={file.type.startsWith('image') ? 'image' : 'video'}>{file.type.startsWith('image') ? '▧' : '▶'}</i>{file.name}</span><span>{file.type.split('/')[1]?.toUpperCase() || 'ФАЙЛ'}</span><span>{sizeOf(file.status === 'done' && file.outputSize ? file.outputSize : file.size)}</span><span className="file-status">{file.status === 'done' ? <a href={file.outputUrl} download={file.outputName}>Скачать</a> : <button disabled={Boolean(running)} onClick={() => compress(file)}>{file.status === 'error' ? 'Повторить' : 'Сжать'}</button>}<button className="remove-button" onClick={() => removeFile(file.id)}>×</button></span></div>)}
